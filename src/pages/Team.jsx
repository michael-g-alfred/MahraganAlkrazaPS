 ...
 * Team.jsx
 * ─────────────────────────────────────────────────────────────────
 * نموذج تسجيل فريق كامل.
 *
 * الخطوات:
 * 1. إدخال اسم الفريق (يجب أن يكون فريداً وغير مكرر).
 * 2. إدخال عدد اللاعبين (من 2 إلى 12).
 * 3. إنشاء نماذج اللاعبين تلقائياً بعد الضغط على "تسجيل اللاعبين".
 * 4. ملء بيانات كل لاعب (نفس حقول الفردي).
 * 5. حفظ جميع اللاعبين دفعة واحدة عبر usePlayerSave.
 *
 * التحقق:
 * - اسم الفريق: لا يتكرر مع الفرق المسجلة مسبقاً.
 * - الاسم الرباعي: فوري عند الكتابة.
 * - عدم التكرار: debounce 600ms لكل لاعب.
 * - تاريخ الميلاد: فوري مع المرحلة المختارة.
 * - الرقم القومي: عبر NationalIdInput داخل Card.
 * ─────────────────────────────────────────────────────────────────
 */
...
/**
 * @param {object}   data              - بيانات الاختيار من Home
 * @param {Function} onUpdateSelection - دالة تحديث الاختيار في الصفحة الأب
 */
export default function Team({ data, onUpdateSelection }) {
  const { loading, saveTeam } = usePlayerSave(data, onUpdateSelection);

  // ── حالات النموذج الرئيسية ────────────────────────────────────

  /** اسم الفريق المدخل */
  const [teamName, setTeamName] = useState("");

  /** عدد اللاعبين المدخل (نص — يُحوَّل لرقم عند الاستخدام) */
  const [playerCount, setPlayerCount] = useState("");

  /** مصفوفة بيانات اللاعبين — تُنشَأ عند الضغط على "تسجيل اللاعبين" */
  const [players, setPlayers] = useState([]);

  /** أخطاء التحقق لكل لاعب { birthdate, name } */
  const [playerErrors, setPlayerErrors] = useState([]);

  /** هل يجري التحقق من الاسم لكل لاعب؟ */
  const [checkingNames, setCheckingNames] = useState([]);

  /** هل الرقم القومي صالح لكل لاعب؟ */
  const [nationalIdValids, setNationalIdValids] = useState([]);

  // جلب اللاعبين الموجودين لمعرفة أسماء الفرق المسجلة مسبقاً
  const [loadingFetch, errorFetch, playersData] = useFetch();

  /** قائمة أسماء الفرق المسجلة مسبقاً (لمنع التكرار) */
  const teamsArr = useMemo(
    () => [...new Set(playersData.map((t) => t.team).filter(Boolean))],
    [playersData],
  );

  // ── حساب صلاحية العدد المدخل ─────────────────────────────────

  const parsedCount = parseInt(playerCount, 10);
  /** هل العدد المدخل بين 2 و12؟ */
  const countIsValid =
    !isNaN(parsedCount) && parsedCount >= 2 && parsedCount <= 12;

  // ── إعادة تعيين اللاعبين عند تغيير العدد ────────────────────
  ...
  /**
   * ينشئ مصفوفات اللاعبين الفارغة بعد التحقق من الاسم والعدد.
   * يُستدعى عند الضغط على "تسجيل اللاعبين".
   */
  ...
  /**
   * يُحدِّث حقل معين للاعب معين ويُشغِّل التحقق المناسب.
   *
   * @param {number} index - رقم اللاعب في المصفوفة
   * @param {string} field - اسم الحقل ("name" | "phone" | "birthdate")
  ...
  const handlePlayerChange = (index, field, value) => {
    ...
    if (field === "birthdate" && data?.stage?.name) {
      // تحقق فوري من تاريخ الميلاد مع المرحلة
    ...
    if (field === "name") {
      // المرحلة الأولى: تحقق فوري من الصيغة الرباعية
      ...
      // المرحلة الثانية: تحقق مؤجل من عدم التكرار (debounce 600ms)
  ...
  /**
   * يُحدِّث الرقم القومي للاعب معين.
  ...
  /**
   * يُحدِّث صلاحية الرقم القومي للاعب معين.
   * يُستدعى من NationalIdInput بعد التحقق.
   */
  ...
  // ── حساب صلاحية النموذج الكامل ──────────────────────────────

  /** هل يوجد أي خطأ في بيانات أي لاعب؟ */
  const hasAnyError =
    playerErrors.some((e) => e?.birthdate || e?.name) ||
    checkingNames.some(Boolean);

  /** هل جميع الأرقام القومية صالحة؟ */
  const allNationalIdsValid =
    nationalIdValids.length > 0 && nationalIdValids.every(Boolean);

  /**
   * هل النموذج كامل وجاهز للحفظ؟
   * يجب أن يكون:
   * - اسم الفريق مكتوب وغير مكرر
   * - جميع اللاعبين مكتملي البيانات
   * - لا أخطاء تحقق
   * - جميع الأرقام القومية صالحة
   */
  const isTeamValid =
    teamName.trim() &&
  ...
  /**
   * يُجري تحققاً نهائياً ثم يحفظ الفريق.
   * يُستدعى عند الضغط على "حفظ الفريق".
   */
  const handleSave = async () => {
    // تحقق نهائي شامل لجميع اللاعبين قبل الإرسال
    ...
    <div
      ...
      <div className="bg-white border-2 border-blue-700 rounded-3xl overflow-hidden shadow-sm">
        ...
        <div className="p-6 grid gap-5">
          {/* ═══ القسم الأول: اسم الفريق وعدد اللاعبين ════════════ */}

          {/* ── اسم الفريق ── */}
          <div>
            <label
              htmlFor="teamName"
              className="block mb-2 text-blue-700 font-semibold text-sm">
              اسم الفريق
            </label>

            {/* عرض الفرق المسجلة مسبقاً كتنبيه */}
            {loadingFetch ?
              <div className="mb-2">
                <Loader size={4} />
              </div>
            : errorFetch ?
              <p role="alert" className="mb-2 text-red-500 text-sm">
                {errorFetch}
              </p>
            : teamsArr.length > 0 && (
            ...
            <input
              ...
              className="w-full border border-blue-700 rounded-xl p-3 text-sm
                         outline-none focus:ring-2 focus:ring-blue-300 transition"
            />

            {/* خطأ تكرار اسم الفريق */}
            {teamsArr.includes(teamName.trim()) && teamName.trim() && (
              <p
                role="alert"
                className="mt-2 text-red-600 text-xs flex items-center gap-1.5">
                <svg
              ...
              {players.map((player, index) => (
                <div key={index}>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    {/* رقم اللاعب في دائرة */}
                    <div
                      className="w-8 h-8 bg-blue-700 text-white rounded-full
                  ...
                  {playerErrors[index]?.birthdate && (
                    <div
                      role="alert"
                      className="mt-3 flex items-start gap-2.5 bg-amber-50 border border-amber-200
